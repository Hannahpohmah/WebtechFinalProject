...
<head>
    ...
    <style>
      ...
      .card {
        ...
        max-width: 50%; /* Limit the width to 30% of the container */
        text-align: center;
        align-items: center;
        border: 3px solid;
        margin-bottom: 10px;
...
<body>
                        ...
                        <div class="row row-cols-1 row-cols-md-3 g-4" >
                            <?php foreach ($vendors as $vendor) : ?>  
                                <?php
                                    ...
                                    $stmt->close();
                                ?>
                                <div class="col vendor-card" data-category="<?php echo $categoryName; ?>" data-name="<?php echo $vendor['fname'] . ' ' . $vendor['lname']; ?>" data-address="<?php echo $vendor['address']; ?>" data-email="<?php echo $vendor['email']; ?>">
    ...
    <script>
        ...
        function searchProvider() {
            var input, filter, providers, name, i;
            var dropdown = document.querySelector('.search-results-dropdown');
            dropdown.innerHTML = ''; // Clear previous results
            input = document.getElementById('searchInput');
            filter = input.value.toUpperCase();
            providers = document.getElementsByClassName('vendor-card');
            var found = false;

            for (i = 0; i < providers.length; i++) {
                name = providers[i].getAttribute('data-name').toUpperCase();
                if (name.indexOf(filter) > -1) {
                    var miniCard = providers[i].cloneNode(true);
                    miniCard.classList.add('mini-vendor-card');
                    miniCard.classList.remove('vendor-card'); // Remove original vendor-card class to avoid style conflicts
            ...
            if (found) {
                ...
                dropdown.style.top = (input.offsetTop + input.offsetHeight) + 'px'; // Position below the search input
                dropdown.style.left = input.offsetLeft + 'px'; // Align with the left edge of the search input
            } else {
